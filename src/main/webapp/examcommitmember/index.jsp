<%@ page language="java" contentType="text/html; charset=EUC-KR"
	pageEncoding="EUC-KR"%>
<!DOCTYPE html>
<html>
<head>
<link
	rel="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css">
<link rel="icon" type="favicon.ico" type="image/x-icon">
<link rel="stylesheet" href="index.css">
<meta charset="EUC-KR">
<link
	href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css"
	rel="stylesheet"
	integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH"
	crossorigin="anonymous">
<title>index</title>
</head>

<body>
<div
	class="header-container d-flex justify-content-between align-items-center p-3">
	<div class="header-left">
		<a href="https://counselors.or.kr/"><img
			src="./img/kca_logo_short.jpg" alt="KCA Logo"></a>
	</div>
	<div class="header-links">
		<a href="https://counselors.or.kr/">한국상담학회</a> <span> · </span> <a
			href="https://counselors.or.kr/"><img src="./img/kca_house.png"
			alt="KCA Logo">인트로</a>
	</div>
	<div class="header-right ml-auto d-flex align-items-center">
		<a href="#">상담이론과 실제A</a> <span class="mx-2"> | </span> <a href="#">로그아웃</a>
		<span class="mx-2"> | </span> <a href="#">마이페이지</a>
	</div>
</div>
<br>
<nav class="navbar navbar-expand-lg bg-body-tertiary">
	<div class="container-fluid">
		<div class="collapse navbar-collapse" id="navbarSupportedContent">
			<ul
				class="navbar-nav me-auto mb-2 mb-lg-0 justify-content-around w-100">
				<li class="nav-item"><a class="nav-link" href="../#">출제 계획서</a></li>
				<li class="nav-item"><a class="nav-link"
					href="../../../biz/getExamPlanList.do">출제</a></li>
				<li class="nav-item"><a class="nav-link" href="../../../biz/getSubtrahendList.do">검토</a></li>
				<li class="nav-item"><a class="nav-link" href="../#">출제
						이원분류표</a></li>
			</ul>
		</div>
	</div>
</nav>
<br>
<div class="container">
	<div class="row">
		<div class="col-lg-3 col-md-6 mb-4">
			<div class="card">
				<img src="" class="card-img-top" alt="...">
				<div class="card-body">
					<h5 class="card-title">1</h5>
					<p class="card-text">출제 계획서</p>
				</div>
			</div>
		</div>
		<div class="col-lg-3 col-md-6 mb-4">
			<div class="card">
				<img src="" class="card-img-top" alt="...">
				<div class="card-body">
					<h5 class="card-title">2</h5>
					<p class="card-text">출제</p>
				</div>
			</div>
		</div>
		<div class="col-lg-3 col-md-6 mb-4">
			<div class="card">
				<img src="" class="card-img-top" alt="...">
				<div class="card-body">
					<h5 class="card-title">3</h5>
					<p class="card-text">검토</p>
				</div>
			</div>
		</div>
		<div class="col-lg-3 col-md-6 mb-4">
			<div class="card">
				<img src="" class="card-img-top" alt="...">
				<div class="card-body">
					<h5 class="card-title">4</h5>
					<p class="card-text">출제 이원분류표</p>
				</div>
			</div>
		</div>
		
		<body>
    <div class="container">
        <div class="grid">
            <div class="button" onclick="location.href='#';">
                <p>출제 계획서</p>
            </div>
            <div class="button" onclick="location.href='#';">
                <p>출제</p>
            </div>
            <div class="button" onclick="location.href='#';">
                <p>검토</p>
            </div>
            <div class="button" onclick="location.href='#';">
                <p>출제 이원분류표</p>
            </div>
        </div>
    </div>
    <script src="scripts.js"></script>
	</div>
</div>
</body>
<footer class="footer">
	<div class="container">
		<div class="row">
			<div class="col-md-12 text-center">
				<ul class="list-unstyled">
					<li>학회소개</li>
					<li>개인정보 취급방침</li>
					<li>이메일 주소 무단수집 거부</li>
					<li>오시는 길</li>
				</ul>
			</div>
		</div>
		<div class="row mt-3">
			<div class="col-md-12 text-center">
				<img src="./img/kca_logo_short.jpg" alt="KCA Logo"></a>
			</div>
		</div>
		<div class="row mt-3">
			<div class="col-md-12 text-center">
				<p>(사)한국상담학회 | 대표자 : 손은령 | 주소 : (06675) 서울특별시 서초구 방배천로 92.
					세양아르비채 102동 404호 | 전화 : 02-875-5830 | 팩스: 02-874-7351</p>
				<p>이메일 : webmaster@counselors.or.kr | 사업자등록번호: 119-82-05101</p>
			</div>
		</div>
	</div>
	<hr>
	<div class="container">
		<div class="row">
			<div class="col-md-12 text-center">
				<p>Copyright ⓒ By Korean Counseling Association. All Rights
					Reserved.</p>
			</div>
		</div>
	</div>
</footer>
</body>
</html>